...
import { createDepartment, listDepartments } from '@/modules/org/department-service';
...
export const GET = withErrorHandler(async (req: NextRequest) => {
  ...
  return Response.json({ data: await listDepartments({ includeDisabled }) });
});

const CreateInput = z.object({ code: z.string().min(1), name: z.string().min(1) });
...
export const POST = withErrorHandler(async (req: NextRequest) => {
  ...
  return Response.json({ data: await createDepartment(input) });
});
