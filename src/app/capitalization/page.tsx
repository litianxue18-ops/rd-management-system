...
export default function CapitalizationListPage() {
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
        <div className="min-w-0">
          ...
          <p className="text-sm md:text-[15px] text-muted-foreground mt-1">
            4 步审批 (研发中心 → 技委会 → 财务部 → 总经理); 5 条件门控 + 必备材料 ≥ 3
          </p>
        </div>
        <Button asChild className="shrink-0">
          <Link href="/capitalization/new">
            ...
            <Table>
              <TableHeader>
                <TableRow className="h-10">
                  ...
                  <TableHead className="text-right">资本化金额</TableHead>
                  ...
                  filtered.map((r) => (
                    <TableRow key={r.id} className="h-10 hover:bg-blue-50/50">
                      ...
                      <TableCell className="py-2 text-right font-medium tabular-nums">
                        {fmtAmount(r.capitalizationAmount)}
                      </TableCell>
                      ...
                      <TableCell className="py-2 text-right space-x-2">
                        <Button size="sm" variant="outline" asChild>
                          <Link href={`/capitalization/${r.id}`}>查看</Link>
                        </Button>
                      </TableCell>
                    </TableRow>
                  ))
              ...
              {filtered.length > 0 && (
                <TableCaption>共 {filtered.length} 份资本化评估</TableCaption>
              )}
            </Table>
          )}
        </CardContent>
